...
如果您想了解更多的的验证方法，请查看[在线API](http://api.hongjuzi.net/hhjslib/hhvalidate "查看验证系列方法API文档")。

## 5 文件工具类示例
本工具类依赖于`RequireJS 2.1.10`所以在本工具类的最上面会有很一堆的`RequireJS`的代码；同时此工具类的加载Js、Css方法有不重复加载的功能，此功能也依赖于`md5.js`的工具库，所以在本工具类开始部分也会有一堆的`md5`相关功能的代码，请不要谎张......如果您是`dwz`或是其它富客户端的用户，那您应该遇到过
<pre>
&lt;script language="text/javascript" src="/path/to/my-script.js"&gt;&lt;/script&gt;
</pre>
这样写是不能调试`my-script.js`中的代码的问题呢？**使用本类库里的`importJs`方法，可以帮助您解决这个问题哦～**
